import { DEFAULT_SILENCE_MS, MAX_ANSWER_MS, nextAfterAnswer, type Env } from "./flow";
...
export class CallSession {
  ...
  private async advance(answeredStep: 1 | 2 | 3): Promise<void> {
    if (!this.callControlId || !this.origin) return;

    try {
      await sendCommand(
        this.callControlId,
        nextAfterAnswer(this.origin, answeredStep),
