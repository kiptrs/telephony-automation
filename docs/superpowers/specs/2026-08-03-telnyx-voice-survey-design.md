...
Date: 2026-08-03
Status: Implemented. Revised 2026-08-03 after the first live call.

## Revision note: gather_using_ai was replaced

The original implementation used `gather_using_ai` with `greeting` omitted, on
the assumption that this made it listen silently. **The first live call proved
that assumption false.** `gather_using_ai` is a conversational LLM assistant,
not a silence detector: omitting `greeting` suppresses only its opening line.
When it judged the `answer` parameter unsatisfied it spoke its own follow-up
question using the default voice `Telnyx.KokoroTTS.af` (Kokoro `af_` = American
Female), and the flow died at round three.

Evidence, from channel analysis of the recording of that call (23.98s, dual
channel):

| Time | Channel | What |
|---|---|---|
| 1.13-2.90 | Telnyx | q1.mp3 |
| 5.71-7.04 | Caller | answer 1 |
| 10.11-11.76 | Telnyx | q2.mp3 |
| 13.69-15.70 | Caller | answer 2 |
| 18.23-22.89 | Telnyx | unattributed TTS speech |
| 23.98 | - | call ended; q3 and thanks never played |

The design now uses the transcription approach that was originally listed as
"considered and rejected". No text-to-speech capability exists anywhere in the
command set, so nothing can speak over the recorded questions.
...
`client_state` decodes to `{ step: 1 | 2 | 3 | "done", phase: "playing" | "listening" }`.

`client_state` persists for the whole call, not just for webhooks generated by
the command that set it. That is what lets `call.transcription` events - which
originate from a single `transcription_start` issued once at call start - still
report the current step.

| Webhook received | Condition | Commands sent |
|---|---|---|
| `call.answered` | — | `record_start` (`dual`, `mp3`), `transcription_start` (`transcription_tracks: "inbound"`, `interim_results: false`), `playback_start` q1 at `{1, playing}` |
| `call.playback.ended` | `step` is 1..3 | `client_state_update` (**PUT**) to `{N, listening}` |
| `call.transcription` | `phase` is `listening`, `is_final`, `step` < 3 | `playback_start` q(N+1) at `{N+1, playing}` |
| `call.transcription` | `phase` is `listening`, `is_final`, `step` == 3 | `playback_start` thanks.mp3 at `{done, playing}` |
| `call.transcription` | `phase` is `playing`, or not final | none |
| `call.playback.ended` | `step` == `"done"` | `hangup` |
| `call.recording.saved` | — | none; log recording URL |
| anything else | — | none |

Two discriminators carry the flow:

- **`step:"done"`** separates the thank-you playback from a question playback;
  both arrive as `call.playback.ended`.
- **`phase`** prevents a transcript that arrives *during* a question from
  advancing past that question. Only `listening` transcripts count. Without
  this, any speech or line noise overlapping a recording would skip it.

`transcription_tracks: "inbound"` means only the caller's audio is transcribed,
so our own playbacks cannot generate transcripts.

Transcription uses Telnyx's own engine with automatic language detection, since
the caller may answer in either English or Lithuanian:
...
The config block repeats `transcription_engine` as its discriminator; Telnyx
rejects it otherwise. `whisper-large-v3-turbo` is chosen over the default
`whisper-tiny` because auto-detection across two languages needs the accuracy,
and because the transcripts are logged as the record of the answers.

Note that `client_state_update` is the one command issued with **PUT**; every
other Telnyx call command is POST.
...
- **The flow advances on the first final transcript.** If the caller pauses
  mid-answer, the speech-to-text engine emits a final result and the next
  question plays over the rest of their sentence. This is the central tradeoff
  of the transcription approach and cannot be fixed without holding per-call
  state and a timer.
- **A silent caller stalls the flow.** There is no equivalent of
  `gather_using_ai`'s `user_response_timeout_ms`. If nobody speaks, no
  transcript is produced and the call sits open until Telnyx's own limits end
  it.
- There is a round-trip gap between `call.playback.ended` and the
  `client_state_update` that opens the listening phase. Answering instantly may
  drop the first word.
